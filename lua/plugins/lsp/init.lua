...
local function lsp_attach_keymap(ev)
  ...
  if client and client.supports_method('textDocument/typeDefinition') then
    vim.keymap.set('n', '<leader><C-]>', vim.lsp.buf.declaration,
  ...
  if client and client.supports_method('callHierarchy/incomingCalls') then
    vim.keymap.set('n', '<leader>lc', vim.lsp.buf.incoming_calls,
      vim.tbl_deep_extend("force", opts, { desc = "Show incoming calls" }))
  end

  if client and client.supports_method('callHierarchy/outgoingCalls') then
    vim.keymap.set('n', '<leader>lC', vim.lsp.buf.outgoing_calls,
      vim.tbl_deep_extend("force", opts, { desc = "Show outgoing calls" }))
  end

  if client and client.supports_method('typeHierarchy/subtypes') then
    vim.keymap.set('n', '<leader>lt', function()
        vim.lsp.buf.typehierarchy("subtypes")
      end,
      vim.tbl_deep_extend("force", opts, { desc = "Show subtypes" }))
  end
  ...
  if client and client.supports_method('typeHierarchy/supertypes') then
    vim.keymap.set('n', '<leader>lT', function()
      ...
      vim.tbl_deep_extend("force", opts, { desc = "Show supertypes" }))
  end

  if client and client.supports_method('textDocument/inlayHint') then
    vim.keymap.set('n', '<leader>lh', vim.lsp.inlay_hint.enable,
      vim.tbl_deep_extend("force", opts, { desc = "Toggle inlay hint" }))
  end

  if client and client.supports_method('textDocument/formatting') then
    vim.keymap.set('n', 'gqq', vim.lsp.buf.format, vim.tbl_deep_extend("force", opts, { desc = "Format" }))
    vim.keymap.set('n', 'gqgq', vim.lsp.buf.format, vim.tbl_deep_extend("force", opts, { desc = "Format" }))
  end
end
